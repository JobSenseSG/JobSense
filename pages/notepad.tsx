...
const parseAIResponse = (responseText: string) => {
  ...
  const individualEmployeeAnalysis: string[] = [];
  ...
  lines.forEach((line) => {
    if (line.startsWith('Recommended Skills')) {
      currentSection = 'skillsRecommendation';
    ...
    } else if (line.startsWith('Individual Employee Analysis')) {
      currentSection = 'individualEmployeeAnalysis';
      ...
      switch (currentSection) {
        ...
        case 'improvementSuggestions':
          ...
          break;
        case 'individualEmployeeAnalysis':
          individualEmployeeAnalysis.push(line);
          break;
...
export default async function handler(
  ...
  try {
    ...
    // Log extractedText to see if it is being populated correctly
    console.log('Extracted Text from PDF:', extractedText);

    // Log other company data to ensure it's being sent to the AI correctly
    ...
    const prompt = `Based on the following company information FOLLOW THE FORMAT:
        ...
        4. Individual Employee Analysis
        - For each employee mentioned in the extracted text, provide one weakness and one improvement suggestion.
        - Use the following format:
        - Employee: Name of the employee.
        - Weakness: Briefly describe the main weakness of the employee.
        - Improvement Suggestion: Provide a specific suggestion for improving this weakness.
        ...
        Employee: John Doe
        Weakness: Limited experience in cloud infrastructure.
        Improvement Suggestion: Enroll in a cloud infrastructure certification course, such as AWS Solutions Architect.
    ...
    // Log the full prompt to debug the data being sent to the AI
    console.log('Generated AI Prompt:', prompt);

    // Make the call to OpenAI's chat completion API
    ...
    // Get and log the response from the AI model
    const responseText =
      chatCompletion.choices?.[0]?.message?.content?.trim() || '';
    console.log('Response from OpenAI:', responseText);

    // Parse the response using the parseAIResponse function
    const analysisData = parseAIResponse(responseText);

    // Send the parsed data as the API response
